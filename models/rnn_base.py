...
class RNNBase(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        assert len(activation) - 1 == len(hidden_size_list), (
            "number of activation should be " "larger by 1 than size of hidden layers."
        )
        assert len(activation) == len(
            layer_type
        ), "number of layer type should equal to the activate"
        activation_dict = {
            "tanh": torch.nn.Tanh,
            "relu": torch.nn.ReLU,
        ...
        layer_dict = {"fc": torch.nn.Linear, "gru": torch.nn.GRU}
        ...
        for ind, item in enumerate(hidden_size_list):
            if self.layer_type[ind] == "fc":
                self.layer_list.append(layer_dict[self.layer_type[ind]](lst_nh, item))
            else:
                self.rnn_num += 1
                self.layer_list.append(
                    layer_dict[self.layer_type[ind]](lst_nh, item, batch_first=True)
                )
        ...
        if self.layer_type[-1] == "fc":
            self.layer_list.append(layer_dict[self.layer_type[-1]](lst_nh, output_size))
        else:
            self.rnn_num += 1
            self.layer_list.append(
                layer_dict[self.layer_type[-1]](lst_nh, output_size, batch_first=True)
            )
        ...
        assert len(self.layer_list) == len(
            self.activation_list
        ), "number of layer should be equal to the number of activation"
    ...
    def meta_forward(
        self, x, hidden_state=None, require_full_hidden=False, aux_state=None
    ):
        _meta_start_time = time.time()
        assert (
            x.shape[-1] == self.input_size
        ), f"inputting size does not match!!!! input is {x.shape[-1]}, expected: {self.input_size}"
        if hidden_state is None:
            hidden_state = self.make_init_state(x.shape[0], x.device)
        assert (
            len(hidden_state) == self.rnn_num
        ), f"rnn num does not match, input is {len(hidden_state)}, expected: {self.rnn_num}"
        x_dim = len(x.shape)
    ...
    def copy_weight_from(self, src_net, tau):
        ...
        with torch.no_grad():
            ...
            for param, target_param in zip(
                src_net.parameters(True), self.parameters(True)
            ):
                target_param.data.copy_(
                    target_param.data * tau + (1 - tau) * param.data
                )
...
def rnn_append_hidden_state(hidden_state: torch.Tensor, data: torch.Tensor):
    for i in range(len(hidden_state)):
        if hidden_state[i] is None:
            hidden_state[i] = data[i]
        elif isinstance(hidden_state[i], tuple):
            hidden_state[i] = (
                torch.cat((hidden_state[i][0], data[i][0]), 1),
...
def rnn_pop_hidden_state(hidden_state: torch.Tensor):
    for i in range(len(hidden_state)):
...
def rnn_get_hidden_length(hidden_state: torch.Tensor):
