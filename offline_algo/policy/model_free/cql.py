...
from offline_algo.policy import SACPolicy
...
class MetaCQLPolicy(SACPolicy):
    ...
    def select_action(
        ...
        with torch.no_grad():
            action, _ = self.actforward(obs, z, deterministic)
    ...
    def calc_pi_values(
        ...
        act, log_prob = self.actforward(obs_pi, z)
        
        obs_to_pred_z_avg = torch.cat((obs_to_pred, z_avg), dim=1) 
        q1 = self.critic1(obs_to_pred_z_avg, act)
        q2 = self.critic2(obs_to_pred_z_avg, act)
    ...
    def calc_random_values(
        ...
        obs_z_avg = torch.cat((obs, z_avg), dim=1) 
        q1 = self.critic1(obs_z_avg, random_act)
        q2 = self.critic2(obs_z_avg, random_act)
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        # TODO: A encoder is needed here
        ################
        if 'z_embeddings' in batch.keys():
            z = batch['z_embeddings']

            # add noise to z_avg
            z_avg = batch['z_mean']
            noise2 = torch.randn_like(z_avg) * 0.005    # noise coef for halfcheetah using model distence: 0.05
            z_avg = z_avg + noise2

            next_z = batch['z_next_embeddings']
            # noise1 = torch.randn_like(z)
            # noise2 = torch.randn_like(z_avg)
            # z = z + noise1
            # z_avg = z_avg + noise2
        else:
            z = torch.randn(batch_size, 2).to("cuda")
            z_avg = torch.randn(batch_size, 2).to("cuda")
            next_z = None
        
        # update actor
        a, log_probs = self.actforward(obss, z)
        ##
        obss_z_avg = torch.cat((obss, z_avg), dim=1) 
        q1a, q2a = self.critic1(obss_z_avg, a), self.critic2(obss_z_avg, a)
        actor_loss = (self._alpha * log_probs - torch.min(q1a, q2a)).mean()
        self.actor_optim.zero_grad()
        actor_loss.backward()
        self.actor_optim.step()
        ...
        # compute td error
        if self._max_q_backup:
            # in default we do not use max_q_backup, thus I have not change this part into a meta-cql version.
            with torch.no_grad():
                tmp_next_obss = next_obss.unsqueeze(1) \
                    .repeat(1, self._num_repeat_actions, 1) \
                    .view(batch_size * self._num_repeat_actions, next_obss.shape[-1])
            ...
            with torch.no_grad():
                next_actions, next_log_probs = self.actforward(next_obss, next_z)
                ##
                next_obss_z_avg = torch.cat((next_obss, z_avg), dim=1) 
                next_q = torch.min(
                    self.critic1_old(next_obss_z_avg, next_actions),
                    self.critic2_old(next_obss_z_avg, next_actions)
                )
                if not self._deterministic_backup:
                    next_q -= self._alpha * next_log_probs

        target_q = rewards + self._gamma * (1 - terminals) * next_q
        q1, q2 = self.critic1(obss_z_avg, actions), self.critic2(obss_z_avg, actions)
        critic1_loss = ((q1 - target_q).pow(2)).mean()
        critic2_loss = ((q2 - target_q).pow(2)).mean()
        ...
        tmp_z = z.unsqueeze(1) \
            .repeat(1, self._num_repeat_actions, 1) \
            .view(batch_size * self._num_repeat_actions, z.shape[-1])
        tmp_next_z = next_z.unsqueeze(1) \
            .repeat(1, self._num_repeat_actions, 1) \
            .view(batch_size * self._num_repeat_actions, next_z.shape[-1])
        tmp_z_avg = z_avg.unsqueeze(1) \
            .repeat(1, self._num_repeat_actions, 1) \
            .view(batch_size * self._num_repeat_actions, z_avg.shape[-1])
        
        obs_pi_value1, obs_pi_value2 = self.calc_pi_values(tmp_obss, tmp_obss, tmp_z, tmp_z_avg)
        next_obs_pi_value1, next_obs_pi_value2 = self.calc_pi_values(tmp_next_obss, tmp_obss, tmp_next_z, tmp_z_avg)
        random_value1, random_value2 = self.calc_random_values(tmp_obss, random_actions, tmp_z, tmp_z_avg)
